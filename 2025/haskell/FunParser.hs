...
-- parse p and ignore the rest
only :: Parser a -> Parser a
only p = p >>= \x -> skip >> return x

-- Returning the result of a deterministic parser that consumes all the input
...
-- parse with p only if it consumes the whole input
...
-- return all the possible parsing of some p's
...
-- parse a p surrounded by (possibly empty) spaces
...
-- When the separator is just a string
