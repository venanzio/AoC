...
readDim input =
  let ls = lines input
      ...
      cs = [ ((x,y,0,0), ls!!y!!x == '#') |
             x <- [0..x1], y <- [0..y1] ]
  in Dim { bottom = (0,0,0,0), top = (x1,y1,0,0), cells = M.fromList cs }
...
neighbours (x,y,z,w) =
  [(x+i,y+j,z+k,w+m) | i<-[-1,0,1], j<-[-1,0,1], k<-[-1,0,1], m <- [-1,0,1],
                       (i,j,k,m) /= (0,0,0,0)]
...
range (x0,y0,z0,w0) (x1,y1,z1,w1) =
  [ (x,y,z,w) | x <- [x0..x1], y <- [y0..y1], z <- [z0..z1], w <- [w0..w1] ]

dimCycle :: Dimension -> Dimension
dimCycle dim =
  let (x0,y0,z0,w0) = bottom dim
      (x1,y1,z1,w1) = top dim
      bt = (x0-1,y0-1,z0-1,w0-1)
      tp = (x1+1,y1+1,z1+1,w1+1)
